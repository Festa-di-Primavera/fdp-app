...
export async function POST({ locals, request }) {
	...
	try {
		...
		const ticketsCount = (await getCountFromServer(ticketsCollection)).data().count;
		if (ticketsCount != body.ticketsNumber) {
			return new Response(JSON.stringify({ message: 'Il numero di biglietti non corrisponde' }), {
				// 400 Bad Request
				status: 400,
				headers: {
		...
		return new Response(JSON.stringify({ message: 'Blocchetti generati con successo' }), {
			status: 200,
			headers: { 'content-type': 'application/json' }
		});
	} catch (e) {
