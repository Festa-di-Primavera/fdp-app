...
export async function GET({ locals, params }) {
	...
	const q = query(collection(getClientDB(), "orders"), where("ticketId", "==", ticketId), where("done", "==", true));
	const orders = await getDocs(q);
	const orderDoc = orders.docs[0];

	if (!orderDoc || !orderDoc.exists()) {
		return new Response(
			JSON.stringify({ message: "Ordine non trovato" }),
			{
				status: 404,
				headers: {
					"Content-Type": "text/plain",
	...
	if (order.items.every((item: OrderItem) => item.ready)) {
		return new Response(
			JSON.stringify({ message: "Ordine già preparato" }),
			{
				status: 400,
				headers: {
					"Content-Type": "text/plain",
