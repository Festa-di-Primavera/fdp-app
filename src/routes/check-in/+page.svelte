<script lang="ts">
	...
	let ticketStatus: 'notFound' | 'alreadyChecked' | 'notSold' | null = $state(null);

	let color: 'green' | 'red' | 'yellow' = $state('green');
	let focus: 'checkIn' | 'newCheckIn' | null = $state(null);
	...
	<div class="flex w-full max-w-96 flex-grow flex-col items-start gap-4 px-5 pb-12 pt-5">
		{#if $user}
			...
			<div class="w-full">
				...
				<InfoCard bind:ticketCode bind:ticket bind:color bind:focus />
