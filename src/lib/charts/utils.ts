...
export function computeSellersStats(tickets: Ticket[]): ChartData {
    ...
    for (const ticket of tickets) {
        if (ticket.seller !== null) {
            sellersStats.set(
                ticket.seller,
                (sellersStats.get(ticket.seller) || 0) + 1
            );
        }
    }
...
export function computeSalesPerHour(tickets: Ticket[]): ChartData {
    ...
    for (const ticket of tickets) {
        if (ticket.soldAt !== null) {
            sellHoursStats.set(
                new Date(ticket.soldAt).getHours(),
                (sellHoursStats.get(new Date(ticket.soldAt).getHours()) || 0) +
                    1
            );
        }
    }
...
export function computeSalesPerTime(
    tickets: Ticket[],
    timeSlot: SalesTimeSlot = SalesTimeSlot.DAY
): ChartData {
    ...
    for (const ticket of tickets) {
        if (ticket.soldAt !== null) {
            ...
            const slotIndex = Math.floor(
                (dateTime.getTime() + 1000 * 60 * 60) / timeSlot
            );
            ...
            timeSlotsMap.set(
                periodStart,
                (timeSlotsMap.get(periodStart) || 0) + 1
            );
        }
    }

    // ordino i timeslot in ordine crescente
    const sortedTimeSlots = Array.from(timeSlotsMap.entries()).sort(
        (a, b) => a[0] - b[0]
    );
    ...
    // per ogni timeslot
    for (
        let indTimeSlot = sortedTimeSlots[0][0];
        indTimeSlot <= sortedTimeSlots[sortedTimeSlots.length - 1][0];
        indTimeSlot += timeSlot
    ) {
        ...
        switch (timeSlot) {
            ...
            case SalesTimeSlot.TWO_WEEKS: {
                ...
                const nextMonth = nextDate.toLocaleString("default", {
                    month: "short",
                });
                label = `${day < 10 ? "0" + day : day} ${month} - ${
...
export function computeCheckInPerTime(
    tickets: Ticket[],
    timeSlot: CheckInTimeSlot = CheckInTimeSlot.HOUR
) {
    ...
    for (const ticket of tickets) {
        if (ticket.checkIn !== null) {
            ...
            checkInsMap.set(
                periodStart,
                (checkInsMap.get(periodStart) || 0) + 1
            );
        }
    }

    const sortedCheckIns = Array.from(checkInsMap.entries()).sort(
        (a, b) => a[0] - b[0]
    );
    ...
    for (
        let indTimeSlot = sortedCheckIns[0][0];
        indTimeSlot <= sortedCheckIns[sortedCheckIns.length - 1][0];
        indTimeSlot += timeSlot
    ) {
        const date = new Date(indTimeSlot);
        ...
        switch (timeSlot) {
            case CheckInTimeSlot.FIFTEEN_MINUTES:
                label = `${day < 10 ? "0" + day : day} ${month},${hour}:${
            ...
            case CheckInTimeSlot.HOUR:
                label = `${day < 10 ? "0" + day : day} ${month}, ${hour}:00-${
                    (hour + 1) % 24
                }:00`;
                break;
