...
import { deleteDoc, doc, getDoc, getDocs, getFirestore, query, setDoc, updateDoc, where, writeBatch } from 'firebase/firestore';
...
export async function validateSessionToken(token: string): Promise<SessionValidationResult> {
	...
	if(Date.now() >= session.expiresAt.getTime()-SESSION_EXPIRES_IN.milliseconds()/2) {
		session.expiresAt = new Date(Date.now() + SESSION_EXPIRES_IN.milliseconds());
...
export async function invalidateExpiredSessions(): Promise<void> {
	const q = query(sessionCollection, where("expiresAt", "<=", new Date(Date.now())));
	const querySnapshot = await getDocs(q);
	const batch = writeBatch(getFirestore());
	querySnapshot.forEach((doc) => batch.delete(doc.ref));
	await batch.commit();
