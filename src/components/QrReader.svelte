<script lang="ts">
	...
	onMount(async () => {
		...
		if (defaultCam && (defaultCam?.length || 0) === 64) updateCamera(defaultCam);
		else {
...
<div class="flex w-full flex-col items-center gap-3">
	...
	>
		<Button
			...
			id="camSelector"
		>
			<CameraIcon class="absolute z-[25] h-6 w-6 text-primary-800 dark:text-primary-300" />
		</Button>
	...
	<span class="text-center font-semibold text-primary-500"
		>{!opened || isPaused ? 'Clicca nel riquadro per scansionare' : 'Scansione in corso'}</span
	>
</div>
{#if devices.length > 0}
	<Dropdown bind:open={camSelectOpen} placement="bottom-start" triggeredBy="#camSelector">
		...
		{#each devices as device}
			<DropdownItem
				...
				class={device.deviceId === selectedCam ? 'text-primary-800 dark:text-primary-200' : ''}
			>
				{device.label}
			</DropdownItem>
		{/each}
